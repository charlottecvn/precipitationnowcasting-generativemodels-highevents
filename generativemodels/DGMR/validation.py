...
import numpy as np
from batchcreator_DGMR import DataGenerator, undo_prep
...
class Evaluator:
    ...
    '''
    def __init__(self, nowcast_method, thresholds = [0, 0.5, 1, 2, 5, 10, 30], 
...
def validate_model(model, run_name, on_test_set = False, random_split=False, resize_method = tf.image.ResizeMethod.BILINEAR):
    if not on_test_set:
        dataset = 'datasets/val_3y_30m.npy'
        data_name = 'val'
        if random_split:
            dataset = 'datasets/val_randomsplit.npy'
            data_name+='_randomsplit'
    else:
        dataset = 'datasets/test_3y_30m.npy'
        data_name = 'test'
        if random_split:
