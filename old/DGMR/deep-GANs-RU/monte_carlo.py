...
def mc_dropout_sampling(training=True):
  # Enable dropout during inference.
  return DGMR(dropout = training, rnn_type = config.rnn_type, x_length = config.x_length, y_length = config.y_length, architecture = config.architecture, g_cycles=config.g_cycles, label_smoothing = config.label_smoothing, l_adv = config.l_adv, l_rec = config.l_rec, norm_method = config.norm_method, downscale256 = config.downscale256, rec_with_mae = config.rec_with_mae, balanced_loss = balanced_loss, rmse_loss = rmse_loss, temp_data = temp_data, SPROG_data = SPROG_data)

model_mc_dropout = mc_dropout_sampling(training=True)
model_mc_dropout.compile(metrics=['mae', 'mae_lower', 'mae_upper'])
print("Fit model")
model_mc_dropout.fit(generator, epochs = config.epochs, callbacks=[WandbCallback()])
