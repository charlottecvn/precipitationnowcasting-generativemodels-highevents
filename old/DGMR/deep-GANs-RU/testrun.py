...
from model_builder_DGMR import DGMR, build_generator
import wandb
...
print(config_DGMR.dir_basic_IDs[:100])
...
run = wandb.init(project='high-precipitation-forecasting',
            ...
            'train_data': config_DGMR.dir_basic_IDs[:100],
            'val_data': config_DGMR.dir_basic_IDs[:100],
            'architecture': 'DGMR',
            'model': 'DGMR',
            'norm_method': 'minmax',
            'downscale256': True,
...
if config.model == 'DGMR':
    model = DGMR(rnn_type = config.rnn_type, x_length = config.x_length, y_length = config.y_length,
             architecture = config.architecture, g_cycles=config.g_cycles, label_smoothing = config.label_smoothing,
               ...
               rec_with_mae = config.rec_with_mae, balanced_loss = balanced_loss, rmse_loss = rmse_loss, temp_data = temp_data, SPROG_data = SPROG_data)
